...
# Runtime pointer files written into the wiki root at init so an agent that
# opens the folder finds its instructions without knowing about ``mdwiki skill``.
# Thin by design: identity, the two things to read first, the hard rules, and
# the trigger phrases. Operating rules stay in the schema and the skill.
POINTER_FILES: dict[str, str] = {
    "claude": "CLAUDE.md",
    "codex": "AGENTS.md",
    "copilot": ".github/copilot-instructions.md",
}
DEFAULT_POINTERS: tuple[str, ...] = ("claude", "codex")
...
class UnknownPointerError(ValueError):
    """Raised when ``pointers`` names a runtime mdwiki does not ship a pointer file for."""
...
class InitResult:
    ...
    pointer_files_written : tuple[str, ...]
        Wiki-root-relative pointer files this call created (``CLAUDE.md`` etc.).
    pointer_files_skipped : tuple[str, ...]
        Requested pointer files left untouched because they already existed.
    """
...
def init_wiki(target: Path, *, profile: str = "working-dir", pointers: tuple[str, ...] | None = None) -> InitResult:
    ...
    pointers : tuple[str, ...], optional
        Runtime pointer files to write into the wiki root (keys of
        ``POINTER_FILES``: ``claude``, ``codex``, ``copilot``). Defaults to
        ``DEFAULT_POINTERS``; pass ``()`` to write none. Files that already
        exist are never overwritten and remain ordinary sources; files this
        call writes are recorded under ``[pointers].files`` so neither init
        nor a later ``refresh`` registers them.
    ...
    UnknownPointerError
        If ``pointers`` names an unsupported runtime.
    """
    ...
    pointer_names = DEFAULT_POINTERS if pointers is None else tuple(pointers)
    unknown = [name for name in pointer_names if name not in POINTER_FILES]
    if unknown:
        raise UnknownPointerError(f"Unknown pointer runtime(s) {', '.join(unknown)}. Available: {', '.join(POINTER_FILES)}.")
    ...
    # Decide the pointer files now so the ones we will write are excluded from
    # source registration below (and from every later ``refresh``). A pointer
    # file that already exists belongs to the user: leave it alone and let it
    # register as an ordinary source.
    pointer_targets = [POINTER_FILES[name] for name in pointer_names]
    pointers_to_write = [rel for rel in pointer_targets if not (target / rel).exists()]
    pointers_skipped = [rel for rel in pointer_targets if (target / rel).exists()]
    if pointers_to_write:
        # Persisted under its own key (not ``exclude.globs``) so user-edited
        # globs such as ``!/*.md`` can never re-include a generated pointer file.
        merged_config = {**merged_config, "pointers": {"files": list(pointers_to_write)}}
    ...
    registered, skipped, dedup_skipped, empty_load_skipped = _register_sources(
        ...
        skip_paths=pointers_to_write,
    )

    for rel in pointers_to_write:
        pointer_path = target / rel
        pointer_path.parent.mkdir(parents=True, exist_ok=True)
        pointer_path.write_text(POINTER_TEMPLATE)
    ...
    pointer_note = ""
    if pointers_to_write:
        pointer_note = f" Wrote pointer file(s): {', '.join(pointers_to_write)}."
    if pointers_skipped:
        pointer_note += f" Left existing pointer file(s) untouched: {', '.join(pointers_skipped)}."
    ...
    return InitResult(
        ...
        wiki_root=target,
        message=(f"Initialized wiki at {target}/{WIKI_DIR_NAME}/. Registered {registered} source(s) as pending" + suffix + pointer_note),
        pointer_files_written=tuple(pointers_to_write),
        pointer_files_skipped=tuple(pointers_skipped),
    )
...
def _register_sources(
    *,
    ...
    skip_paths : Sequence[str], optional
        Wiki-root-relative POSIX paths that are never sources regardless of
        globs (the runtime pointer files ``init`` wrote, persisted under
        ``[pointers].files``). Counted as ``skipped``.
    ...
    with connect(db_path) as conn:
        for source_path in source_paths:
            try:
                ...
                if rel_posix in always_skip or (spec is not None and spec.match_file(rel_posix)) or config_spec.match_file(rel_posix):
                    skipped += 1
                    continue
